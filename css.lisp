...
(defmethod embeddable-value-form ((language css) form environment)
...
(defun process-non-import-css (language processor sexp)
  (destructuring-bind (selector &rest attributes) sexp
    ...
    (loop for (k v) on attributes by #'cddr do
         (process-css-key-or-value language processor k)
         (raw-string processor ": ")
         (process-css-key-or-value language processor v)
         (raw-string processor ";")
         (freshline processor))
    (unindent processor)
    (freshline processor)
  ...
  (cond
    ...
    (t
     (multiple-value-bind (tag class pseudo-class id) (parse-selector selector)
       (when tag
         (raw-string processor (string-downcase tag)))
       (when class
...
(defun process-css-key-or-value (language processor form)
  (if (keywordp form)
    (raw-string processor (string-downcase form))
    (process language processor form nil)))
